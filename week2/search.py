...
def autocomplete():
    ...
    if request.method == 'GET':  # a query has been submitted
        ...
        if prefix is not None:
            ...
            ##### W2, L3, S1
            type_to_index = {"products": "bbuy_products", "queries": "bbuy_queries"}
            suggest = {
                "suggest": {
                    "autocomplete": {
                        "prefix": prefix,
            ...
            index_name = type_to_index.get(_type)
            search_response = get_opensearch().search(body=suggest, index=index_name)
